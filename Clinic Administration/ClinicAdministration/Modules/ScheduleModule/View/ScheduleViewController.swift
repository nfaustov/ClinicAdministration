...
final class ScheduleViewController: UIViewController {
    typealias PresenterType = SchedulePresentation
    ...
    private var actionList: [ScheduleAction] = [.showNextSchedule, .showAllSchedules, .editSchedule]
    ...
    private func createDataSource() {
        dataSource = UICollectionViewDiffableDataSource<Section, AnyHashable>(
            ...
            let factory = ScheduleCellFactory(collectionView: collectionView)
    ...
    private func createCompositionalLayout(withSchedules: Bool, count: Int = 0) -> UICollectionViewLayout {
        let layout = UICollectionViewCompositionalLayout { sectionIndex, _ in
            ...
            let timeTableLayout = ScheduleCollectionViewLayout(
...
extension ScheduleViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let doctorSchedule = dataSource.itemIdentifier(for: indexPath) as? DoctorSchedule {
            ...
            if let patient = patientAppointment.patient {
                presenter.showPatientCard(patient)
            } else if let selectedSchedule = selectedSchedule {
                presenter.registerPatient(schedule: selectedSchedule, selectedAppointment: patientAppointment)
            }
        } else if let action = dataSource.itemIdentifier(for: indexPath) as? ScheduleAction,
